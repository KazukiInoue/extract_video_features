...
void extract4608HsvFeatures() {
	...
	string rootDir[2] = { "C:/MUSIC_RECOMMENDATION/src_data/shots_OMV200/",
						 "../../shot_detection/output_first_frames_of_shots/recommendation_test/" };

	string toDir[2] = { "C:/MUSIC_RECOMMENDATION/src_data/train_features/OMV200_csv_4608hsv/",
						"../output_video_features/shot/recommendation_test/4608hsv_features/" };

	// ヒストグラムのbinの数を指定
	const int bin[3] = { 18,  16, 16 };

	for (int categoryItr = 0; categoryItr < sizeof rootDir / sizeof rootDir[0]; categoryItr++) {
		...
		for (int videoItr = 0; videoItr < videoList.size(); videoItr++) {
			if (videoList[videoItr] != "." && videoList[videoItr] != "..") {
				...
				// csvで出力
				string csv_name = toDir[categoryItr] + videoList[videoItr] + "_shot_hsv4608.csv";
